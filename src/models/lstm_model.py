...
def train_lstm(processed_file="data/preprocessed_dataset.csv",
    ...
    model.fit(
        ...
        verbose=0,  # turn off default keras progress
    ...
    # ----------------------------
    # 8. Forecast next 24 hours (5-min intervals)
    # ----------------------------
    last_seq = demand_scaled[-seq_length:]
    input_seq = np.expand_dims(last_seq, axis=0)
    forecast_scaled = []

    steps_5min = 24 * 12  # 24 hours * 12 = 288 steps of 5-min each
    for _ in range(steps_5min):
    ...
    # Create forecast DataFrame (5-min)
    forecast_df_5min = pd.DataFrame({
        "ds": pd.date_range(start=data["datetime"].iloc[-1] + pd.Timedelta(minutes=5),
                            periods=steps_5min, freq='5min'),
        "yhat": forecast.flatten()
    })
    
    # ----------------------------
    # 9. Aggregate hourly for visualization
    # ----------------------------
